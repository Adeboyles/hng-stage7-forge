...
def load_webhook_url() -> str:
    config_path = Path(os.getenv("FORGE_CONFIG_PATH", "config.yaml"))
...
async def notify_pipeline_succeeded(pipeline_name: str, run_id: str, duration: str):
    await send_slack(
        {
...
async def notify_pipeline_failed(
    pipeline_name: str, run_id: str, duration: str, failing_job: str
):
    await send_slack(
        {
...
async def notify_integrity_failure(
    artifact: str, expected_sha: str, actual_sha: str, run_id: str
):
    await send_slack(
        {
...
async def notify_resolution_failure(pipeline_name: str, run_id: str, details: str):
    await send_slack(
        {
